...
import {
  call_exports,
  mini_exports
} from "./chunk-7LVFCDHV.js";
import "./chunk-W3VEH4UB.js";
...
import {
  ...
  useReconnect,
  useSwitchAccount
} from "./chunk-3WVCRVFM.js";
import "./chunk-KME7UKT3.js";
import "./chunk-W4UUC32R.js";
import {
  arbitrum,
  arbitrumSepolia,
  base,
...
var require_eventemitter3 = __commonJS({
  "node_modules/@gemini-wallet/core/node_modules/eventemitter3/index.js"(exports, module) {
    "use strict";
...
// node_modules/@privy-io/wagmi/dist/esm/index.js
  ...
  return createConnector((config) => ({
    ...
    async getProvider() {
      if (!walletProvider) {
        ...
        const { createCoinbaseWalletSDK } = await import("./dist-JZMKMS56.js");
    ...
    async getProvider() {
      ...
      if (!walletProvider) {
        const CoinbaseWalletSDK = await (async () => {
          const { default: SDK } = await import("./dist-UGAACCZQ.js");
...
function getChecksumAddressUnmemoized(hexAddress) {
  ...
  for (let i2 = 0; i2 < length; i2++) {
    const byteIndex = i2 >> 1;
    const nibbleIndex = i2 & 1;
    const byte = hashBytes[byteIndex];
    const nibble = nibbleIndex === 0 ? byte >> 4 : byte & 15;
    result[i2] = nibble >= 8 ? address[i2].toUpperCase() : address[i2];
  }
...
// node_modules/@metamask/rpc-errors/node_modules/@metamask/utils/dist/superstruct.mjs
...
// node_modules/@metamask/rpc-errors/node_modules/@metamask/utils/dist/unitsConversion.mjs
var zero = BigInt(0);
...
var safeJsonStringify = (obj) => JSON.stringify(obj, (_2, value) => typeof value === "bigint" ? value.toString() + "n" : value, 2);
    ...
    async getProvider() {
      async function initProvider() {
        ...
        const MetaMaskSDK = await (async () => {
          const { default: SDK } = await import("./metamask-sdk-3YCXGDSL.js");
          if (typeof SDK !== "function" && typeof SDK.default === "function")
            return SDK.default;
          return SDK;
        })();
  ...
  return createConnector((config) => ({
    ...
    async getProvider() {
      ...
      if (!isIframe)
        return;
      if (!provider_) {
        const { default: SDK } = await import("./esm-TH4GFZDZ.js");
        ...
        const SafeAppProvider = await (async () => {
          const Provider = await import("./dist-VTISLIDN.js");
    ...
    async getProvider({ chainId } = {}) {
      ...
      async function initProvider() {
        ...
        if (!optionalChains.length)
          return;
        const { EthereumProvider } = await import("./index.es-RVW5JR6H.js");
...
var v = ({ setActiveWalletForWagmi: e }) => {
  let { wallets: t, ready: o2 } = o(), { user: r } = m(), i2 = useConfig(), { reconnect: n2 } = useReconnect();
  n({ onSuccess: async (c) => {
    var _a, _b;
    await ((_a = i2.storage) == null ? void 0 : _a.removeItem(`${c.meta.id}.disconnected`)), e || (await ((_b = i2.storage) == null ? void 0 : _b.setItem("recentConnectorId", c.meta.id)), n2());
  } }), i({ onComplete: async (c, s, a, l, m2) => {
    var _a;
    !m2 || m2.type === "wallet" || !c.linkedAccounts.find((u) => u.type === "wallet" && u.walletClientType === "privy") || await ((_a = i2.storage) == null ? void 0 : _a.removeItem("io.privy.wallet.disconnected"));
  } }), (0, import_react.useEffect)(() => {
    if (!e) return;
    (async () => {
      let s = e({ wallets: t, user: r });
      if (!s) {
        x(i2), n2();
        return;
      }
      let [a] = await C([s], i2);
      if (!a) {
        console.error(`Failed to setup connector for ${s.address}`);
        return;
      }
      await _(s, a, i2), n2();
    })();
  }, [t, r, e]), (0, import_react.useEffect)(() => {
    e || C(t, i2).then(() => {
      o2 && n2();
    });
  }, [t, !e, o2]);
...
var C = async (e, t) => {
  let o2 = await Promise.all(e.map(async (r) => {
    let i2 = await r.getEthereumProvider(), n2 = t.connectors.find((s) => s.id === r.meta.id);
    if (n2) return n2;
    let c = injected({ target: { provider: i2, id: r.meta.id, name: r.meta.name, icon: r.meta.icon } });
    return t._internal.connectors.setup(c);
  }));
  return t._internal.connectors.setState(o2), o2;
};
var x = (e) => {
  e._internal.connectors.setState([]), e.setState((t) => ({ chainId: t.chainId, connections: /* @__PURE__ */ new Map(), current: void 0, status: "disconnected" }));
};
var _ = async (e, t, o2) => {
  var _a, _b;
  await Promise.all([(_a = o2.storage) == null ? void 0 : _a.removeItem(`${e.meta.id}.disconnected`), (_b = o2.storage) == null ? void 0 : _b.setItem("recentConnectorId", t.id)]);
  let r = Number(e.chainId.replace("eip155:", ""));
  o2.chains.find((n2) => n2.id === r) || (r = o2.chains[0].id);
  let i2 = /* @__PURE__ */ new Map([[t.uid, { accounts: [e.address], chainId: r, connector: t }]]);
  o2.setState((n2) => ({ ...n2, chainId: r, connections: i2, current: t.uid, status: "connected" }));
};
var y = ({ children: e, setActiveWalletForWagmi: t }) => (v({ setActiveWalletForWagmi: t }), (0, import_jsx_runtime.jsx)(import_jsx_runtime.Fragment, { children: e }));
var T = ({ children: e, setActiveWalletForWagmi: t, ...o2 }) => (0, import_jsx_runtime2.jsx)(WagmiProvider, { reconnectOnMount: false, ...o2, children: (0, import_jsx_runtime2.jsx)(y, { setActiveWalletForWagmi: t, children: e }) });
var R = (e) => {
  var _a;
  return createConfig({ ssr: true, ...e, connectors: (_a = e.connectors) == null ? void 0 : _a.filter((t) => t.type === "mock"), multiInjectedProviderDiscovery: false });
};
var p = () => {
  let { switchAccount: e } = useSwitchAccount(), { connect: t } = useConnect(), o2 = useConfig();
  return { setActiveWallet: (0, import_react2.useCallback)(async (i2) => {
    var _a;
    for (let n2 of o2.connectors) if (n2.id === i2.meta.id && (await n2.getAccounts()).includes(i2.address)) {
      await ((_a = o2.storage) == null ? void 0 : _a.removeItem(`${i2.meta.id}.disconnected`)), o2.state.connections.get(n2.uid) ? e({ connector: n2 }) : t({ connector: n2 });
      return;
    }
  }, [o2.connectors, o2.state.connections, t, e]) };
};
var U = () => {
  let { setActiveWallet: e } = p();
  return { setActiveWallet: e, activeWallet: void 0 };
};
var z = ({ getSmartAccountFromSigner: e }) => {
  let t = useConnectors(), o2 = useConfig(), { reconnect: r } = useReconnect();
  (0, import_react3.useEffect)(() => {
    (async () => {
      var _a;
      if (t.find((d) => d.id === "io.privy.smart_wallet")) return;
      let c = t.find((d) => d.id === "io.privy.wallet");
      if (!c) return;
      let s = await c.getProvider(), a = await e({ signer: s }), l = injected({ target: { provider: a, id: "io.privy.smart_wallet", name: "io.privy.smart_wallet", icon: "" } }), m2 = o2._internal.connectors.setup(l);
      o2._internal.connectors.setState([m2]), await ((_a = o2.storage) == null ? void 0 : _a.setItem("recentConnectorId", m2.id)), r();
    })();
  }, [t, o2, e]);
};
export {
  T as WagmiProvider,
  R as createConfig,
  z as useEmbeddedSmartAccountConnector,
  U as usePrivyWagmi,
  p as useSetActiveWallet
};
...
lodash/lodash.js:
   ...
   *)
*/
//# sourceMappingURL=@privy-io_wagmi.js.map
