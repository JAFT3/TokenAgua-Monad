...
function serializeTransactionEIP712(transaction) {
  const { chainId, gas, nonce: nonce2, to, from: from15, value, maxFeePerGas, maxPriorityFeePerGas, customSignature, factoryDeps, paymaster, paymasterInput, gasPerPubdata, data } = transaction;
...
function transactionToMessage(transaction) {
  const { gas, nonce: nonce2, to, from: from15, value, maxFeePerGas, maxPriorityFeePerGas, factoryDeps, paymaster, paymasterInput, gasPerPubdata, data } = transaction;
  return {
    txType: 113n,
    from: BigInt(from15),
    to: to ? BigInt(to) : 0n,
...
function serializeTransactionDeposit(transaction) {
  ...
  const { sourceHash, data, from: from15, gas, isSystemTx, mint: mint2, to, value } = transaction;
  const serializedTransaction = [
    sourceHash,
    from15,
    to ?? "0x",
...
// node_modules/viem/node_modules/ox/_esm/tempo/AuthorizationTempo.js
...
// node_modules/viem/node_modules/ox/_esm/tempo/SignatureEnvelope.js
...
function serialize(envelope, options = {}) {
  ...
  if (type2 === "secp256k1") {
    const secp256k1 = envelope;
    return concat(toHex3(secp256k1.signature), options.magic ? magicBytes : "0x");
  }
  if (type2 === "p256") {
    const p2562 = envelope;
    return concat(serializedP256Type, fromNumber(p2562.signature.r, { size: 32 }), fromNumber(p2562.signature.s, { size: 32 }), fromNumber(p2562.publicKey.x, { size: 32 }), fromNumber(p2562.publicKey.y, { size: 32 }), fromNumber(p2562.prehash ? 1 : 0, { size: 1 }), options.magic ? magicBytes : "0x");
  }
  if (type2 === "webAuthn") {
    const webauthn = envelope;
    const webauthnData = concat(webauthn.metadata.authenticatorData, fromString(webauthn.metadata.clientDataJSON));
    return concat(serializedWebAuthnType, webauthnData, fromNumber(webauthn.signature.r, { size: 32 }), fromNumber(webauthn.signature.s, { size: 32 }), fromNumber(webauthn.publicKey.x, { size: 32 }), fromNumber(webauthn.publicKey.y, { size: 32 }), options.magic ? magicBytes : "0x");
  }
  if (type2 === "keychain") {
    const keychain = envelope;
    const keychainTypeId = keychain.version === "v1" ? serializedKeychainType : serializedKeychainV2Type;
    return concat(keychainTypeId, keychain.userAddress, serialize(keychain.inner), options.magic ? magicBytes : "0x");
  }
  throw new CoercionError({ envelope });
}
function toRpc3(envelope) {
  ...
  if (type2 === "p256") {
    ...
    return {
      ...
      pubKeyX: fromNumber(p2562.publicKey.x, { size: 32 }),
      pubKeyY: fromNumber(p2562.publicKey.y, { size: 32 }),
      r: fromNumber(p2562.signature.r, { size: 32 }),
      s: fromNumber(p2562.signature.s, { size: 32 }),
      type: "p256"
    };
  }
  if (type2 === "webAuthn") {
...
// node_modules/viem/node_modules/ox/_esm/tempo/TempoAddress.js
...
// node_modules/viem/node_modules/ox/_esm/tempo/KeyAuthorization.js
...
function toRpc5(authorization) {
  const { address, scopes, chainId, expiry, limits, type: type2, signature } = authorization;
  const allowedCalls = (() => {
    if (!scopes)
      return void 0;
...
// node_modules/viem/node_modules/ox/_esm/tempo/Period.js
var Period_exports = {};
...
// node_modules/viem/node_modules/ox/_esm/tempo/PoolId.js
var PoolId_exports = {};
...
function fromAddress(address) {
  ...
  return toBigInt(slice(resolved, tip20Prefix.length));
}
function toAddress(tokenId) {
  ...
  return concat(tip20Prefix, tokenIdHex);
}
function compute(value) {
  ...
  return toBigInt(slice(hash5, 0, 8));
}
...
// node_modules/viem/node_modules/ox/_esm/tempo/RpcSchemaTempo.js
var RpcSchemaTempo_exports = {};

// node_modules/viem/node_modules/ox/_esm/tempo/Tick.js
var Tick_exports = {};
...
// node_modules/viem/node_modules/ox/_esm/tempo/TokenRole.js
...
// node_modules/viem/node_modules/ox/_esm/tempo/Transaction.js
...
function toRpc7(transaction, _options) {
  ...
  if (transaction.feePayerSignature) {
    ...
    rpc.feePayerSignature.v = fromNumber(yParityToV((_a2 = transaction.feePayerSignature) == null ? void 0 : _a2.yParity));
  }
  if (transaction.signature)
...
// node_modules/viem/node_modules/ox/_esm/tempo/TransactionReceipt.js
...
// node_modules/viem/node_modules/ox/_esm/tempo/TransactionRequest.js
...
function fromRpc9(request) {
  ...
  if (typeof request.gas !== "undefined")
    request_.gas = toBigInt(request.gas);
  if (typeof request.gasPrice !== "undefined")
    request_.gasPrice = toBigInt(request.gasPrice);
  if (typeof request.maxFeePerBlobGas !== "undefined")
    request_.maxFeePerBlobGas = toBigInt(request.maxFeePerBlobGas);
  if (typeof request.maxFeePerGas !== "undefined")
    request_.maxFeePerGas = toBigInt(request.maxFeePerGas);
  if (typeof request.maxPriorityFeePerGas !== "undefined")
    request_.maxPriorityFeePerGas = toBigInt(request.maxPriorityFeePerGas);
  if (typeof request.nonce !== "undefined")
    request_.nonce = toBigInt(request.nonce);
  if (typeof request.type !== "undefined")
    request_.type = fromRpcType2[request.type] || request.type;
  if (typeof request.value !== "undefined")
    request_.value = toBigInt(request.value);
  return request_;
}
...
function fromRpc10(request) {
  ...
  if (request.calls)
    request_.calls = request.calls.map((call) => {
      ...
      if (call.value && call.value !== "0x")
        mapped.value = toBigInt(call.value);
      return mapped;
    });
  ...
  if (typeof request.nonceKey !== "undefined")
    request_.nonceKey = toBigInt(request.nonceKey);
  return request_;
}
...
// node_modules/viem/node_modules/ox/_esm/tempo/TxEnvelopeTempo.js
...
// node_modules/viem/node_modules/ox/_esm/core/TxEnvelope.js
  ...
  if (!transaction.from && signatureEnvelope) {
    try {
      transaction.from = extractAddress({
        payload: getSignPayload3(from11(transaction)),
        signature: signatureEnvelope,
        root: true
      });
    } catch {
...
function from11(envelope, options = {}) {
...
function serialize4(envelope, options = {}) {
    ...
    if (options.format === "feePayer" && signature) {
      ...
      if (sig.type === "secp256k1")
        return recoverAddress({
          payload: getSignPayload3(from11(envelope)),
  ...
  return concat(options.format === "feePayer" ? feePayerMagic : serializedType, fromHex3(serialized));
}
function getSignPayload3(envelope, options = {}) {
  const sigHash = hash3(envelope, { presign: true });
  if (options.from)
    return keccak2562(concat("0x04", sigHash, resolve(options.from)));
  return sigHash;
}
...
// node_modules/viem/node_modules/ox/_esm/tempo/VirtualAddress.js
...
function from12(value) {
  return from3(concat(toFixedHex(value.masterId, 4), magic, toFixedHex(value.userTag, 6)));
}
function isVirtual(address) {
  try {
    const resolved = resolveAddress(address);
    return slice(resolved, 4, 14).toLowerCase() === magic;
  } catch {
    return false;
  }
}
function parse2(address) {
  const resolved = resolveAddress(address);
  if (slice(resolved, 4, 14).toLowerCase() !== magic)
    throw new InvalidMagicError({ address: resolved });
  return {
    masterId: slice(resolved, 0, 4),
    userTag: slice(resolved, 14, 20)
  };
}
function validate7(address) {
  try {
    parse2(address);
    return true;
  } catch {
...
function toFixedHex(value, size2) {
  if (typeof value === "number" || typeof value === "bigint")
    return fromNumber(value, { size: size2 });
  if (typeof value === "string") {
    assert(value, { strict: true });
    return padLeft(value, size2);
  }
  return fromBytes(padLeft2(value, size2));
}

// node_modules/viem/node_modules/ox/_esm/tempo/VirtualMaster.js
...
// node_modules/viem/node_modules/ox/_esm/tempo/internal/virtualMasterPool.js
var cachedPool;
var _a;
...
async function resolveNode() {
  ...
  const { wasmBase64 } = await import("./mine.wasm-NVCDBISA.js");
...
function getRegistrationHash(value) {
  return keccak2562(concat(resolveAddress2(value.address), toFixedHex2(value.salt, 32)));
}
function getMasterId(value) {
  return slice(getRegistrationHash(value), 4, 8);
}
function validateSalt(value) {
  try {
    return hasProofOfWork(keccak2562(concat(resolveAddress2(value.address), toFixedHex2(value.salt, 32)), { as: "Bytes" }));
  } catch {
    return false;
  }
}
function mineSalt(value) {
  ...
  assertCount(count);
  const addressBytes = fromHex(resolveAddress2(value.address));
...
async function mineSaltAsync(parameters) {
  ...
  const address = resolveAddress2(parameters.address);
...
function resolveAddress2(address) {
  const resolved = resolve(address);
  assert2(resolved, { strict: false });
...
// node_modules/viem/node_modules/ox/_esm/tempo/ZoneId.js
var ZoneId_exports = {};
...
// node_modules/viem/node_modules/ox/_esm/tempo/ZoneRpcAuthentication.js
...
var fieldsSize = 29;
var version = 0;
function from13(authentication, options = {}) {
...
function deserialize4(serialized) {
  ...
  const signature = slice(serialized, 0, fieldsOffset);
  const fields = slice(serialized, fieldsOffset);
  const parsedVersion = toNumber(slice(fields, 0, 1), { size: 1 });
  ...
  return {
    chainId: toNumber(slice(fields, 5, 13), { size: 8 }),
    expiresAt: toNumber(slice(fields, 21, 29), { size: 8 }),
    issuedAt: toNumber(slice(fields, 13, 21), { size: 8 }),
    signature: deserialize(signature),
    version,
    zoneId: toNumber(slice(fields, 1, 5), { size: 4 })
  };
}
function getFields(authentication) {
  return concat(fromNumber(version, { size: 1 }), fromNumber(authentication.zoneId, { size: 4 }), fromNumber(authentication.chainId, { size: 8 }), fromNumber(authentication.issuedAt, { size: 8 }), fromNumber(authentication.expiresAt, { size: 8 }));
}
function getSignPayload4(authentication, options = {}) {
  const authHash = hash4(authentication);
  if (options.userAddress)
    return keccak2562(concat("0x04", authHash, resolve(options.userAddress)));
  return authHash;
}
function hash4(authentication) {
  return keccak2562(concat(magicBytes2, getFields(authentication)));
}
function serialize5(authentication, options = {}) {
  const signature = options.signature || authentication.signature;
  if (!signature)
    throw new MissingSignatureError();
  return concat(serialize(from6(signature)), getFields(authentication));
}
...
var chainConfig5 = {
    ...
    async (r, { phase }) => {
      ...
      const useExpiringNonce = await (async () => {
        ...
        if (request.feePayer && typeof request.nonceKey === "undefined")
          return true;
        const address = (_a3 = request.account) == null ? void 0 : _a3.address;
      ...
      if (!request.feeToken && ((_c = request.chain) == null ? void 0 : _c.feeToken))
  ...
  async verifyHash(client, parameters) {
    const { address, hash: hash5, signature, mode: mode2 } = parameters;
    const envelope = (() => {
    ...
    if (envelope) {
      if ((envelope == null ? void 0 : envelope.type) === "keychain" && mode2 === "allowAccessKey") {
        const accessKeyAddress = fromPublicKey(from2(envelope.inner.publicKey));
        ...
        if (keyInfo.expiry <= BigInt(Math.floor(Date.now() / 1e3)))
          return false;
        const innerPayload = envelope.version === "v2" ? keccak256(concat("0x04", hash5, address)) : hash5;
...
//# sourceMappingURL=chunk-H5XNZUB6.js.map
